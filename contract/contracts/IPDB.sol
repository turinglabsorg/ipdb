...
contract IPDB {
    ...
    function store(string memory name, string memory cid) {
        ...
        databases[msg.sender][name][versions[msg.sender][name]].timestamp = block.timestamp;
    }

    // This function returns the latest database
    function get(address identifier, string memory name) returns (string memory) {
        return databases[identifier][name][versions[identifier][name]].cid;
    }

    // This function searches for a specific version of database
    function search(address identifier, string memory name, uint256 version) returns (string memory) {
